...
class AdminRepeatVisit extends Page implements HasTable
{
    ...
    protected static ?string $slug = 'reporting/monthly/admin/repeat-visit';

    protected static ?string $navigationLabel = 'Repeat Visit';

    protected static ?string $title = 'Admin Repeat Visit';

    protected ?string $heading = 'Repeat Visit';

    public function getTableRecordKey($record): string
    {
        return (string) $record->getKeyName();
    }
    protected static string $view = 'filament.pages.repeat-visit';
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->query(Reporting::query()
                    ...
                    ->joinSub(
                        Outstanding::select('id')
                            ->withCount('reportings')
                            ->having('reportings_count', '>', 2),
                        'filtered_outstandings',
                        'filtered_outstandings.id',
                        '=',
                        'reportings.outstanding_id'
                    )
